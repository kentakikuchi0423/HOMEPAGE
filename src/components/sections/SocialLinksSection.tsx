...
const platformConfig: Record<string, { label: string; iconBg: string; iconText: string; icon: string }> = {
  X: {
    label: 'X (Twitter)',
    iconBg: 'bg-gray-100',
          ...
          {sorted.map(({ platform, url }, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${config.iconBg} ${config.iconText}`}>
                    {config.icon}
                  </span>
                  {config.label}
                </a>
